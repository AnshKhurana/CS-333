...
int main(int argc, char *argv[])
{
  ...
  if(argc < 3) {
    printf("Need 2 commandline arguments: #readers, #writers\n");
    exit(1);
  }
  ...
  for(int i=0;i<num_threads; i++)
	...
	if ((read_num_threads > 0) && (*max_reader_acquire_time > *min_reader_release_time)){
	  printf("ERROR: Reader should not wait to acquire lock\n");
	  exit(0);
	}
	...
	if ((read_num_threads > 0) && (write_num_threads > 0) && (*min_writer_acquire_time < *max_reader_release_time)){
	  printf("ERROR: All readers must get lock before any writer\n");
	  exit(0);
	}
	...
	for (int i = 0; i < write_num_threads; i++)
	  if ((writerReleaseTime[i] - writerAcquireTime[i]) != 1){
	    printf("ERROR: No reader/ writer is allowed when a writer holds lock\n");
	    exit(0);
	  }
